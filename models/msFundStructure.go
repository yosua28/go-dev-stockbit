...
func AdminGetListMsFundStructure(c *[]MsFundStructure, limit uint64, offset uint64, params map[string]string, nolimit bool) (int, error) {
	query2 := `SELECT
				ms_fund_structure.* FROM 
				ms_fund_structure `
	query := query2 + " WHERE ms_fund_structure.rec_status = 1 "
	...
	for field, value := range params {
		if !(field == "orderBy" || field == "orderType") {
			whereClause = append(whereClause, "ms_fund_structure."+field+" = '"+value+"'")
		}
	}
