...
func AdminGetListScRole(c echo.Context) error {
	var err error
	var status int
	...
	status, err = models.GetAllScRole(&scRole, limit, offset, params, noLimit)
	if err != nil {
		log.Error(err.Error())
		return lib.CustomError(status, err.Error(), "Failed get data")
	}

	var responseData []models.ScRoleInfoLogin

	for _, role := range scRole {
		var data models.ScRoleInfoLogin
